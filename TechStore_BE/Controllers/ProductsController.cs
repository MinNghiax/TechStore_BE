...
namespace TechStore_BE.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public async Task<ActionResult> PostProduct([FromBody] Products product)
        {
            ...
            return Ok(new { Message = "Thêm sản phẩm thành công!", newProduct.product_id});
        }
        ...
        public JsonResult SaveFile()
        {
            try
            {
                ...
                var savePath = Path.Combine(_env.WebRootPath, "images", fileName); //connect root url web (wwwroot) with images folder

                using (var stream = new FileStream(savePath, FileMode.Create)) //read, write file 
                {
                    file.CopyTo(stream); //read, write
                }
